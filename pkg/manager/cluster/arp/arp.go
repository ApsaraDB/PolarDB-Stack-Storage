...
func getInterface(ip string) (*net.Interface, error) {
	...
	for idx, addr := range netInterfaces {
		addrs, err := addr.Addrs()
		smslog.Infof("net interfaces: %v", addrs)
		if err != nil {
			continue
		}
		for _, ipAddr := range addrs {
			if isEqual(ipAddr.String(), ip) {
				return &netInterfaces[idx], nil
			}
		}
	}
	return nil, fmt.Errorf("not fount mac addr for ip %s", ip)
}

func isEqual(src, tgt string) bool {
	src = strings.Split(src, "/")[0]
	src = strings.Split(src, ":")[0]
	tgt = strings.Split(tgt, "/")[0]
	tgt = strings.Split(tgt, ":")[0]
	return src == tgt
}

// ARPSendGratuitous sends a gratuitous ARP message via the specified interface.
func ARPSendGratuitous(address string, iface Iface) error {
	_iface, err := net.InterfaceByName(string(iface))
	if err != nil {
	...
	smslog.Infof("Broadcasting ARP update for %s (%s) via %s", address, _iface.HardwareAddr, _iface.Name)
	m, err := gratuitousARP(ip, _iface.HardwareAddr)
	if err != nil {
		return err
	}
	return sendARP(_iface, m)
}

func ARPSendGratuitousByIp(vip string, currentIp string) error {
	iface, err := getInterface(currentIp)
	if err != nil {
		smslog.Errorf("not found mac addr %s", err.Error())
		return err
	}
	ip := net.ParseIP(vip)
	if ip == nil {
	...
	smslog.Infof("Broadcasting ARP update for %s (%s)", vip, iface.HardwareAddr)
